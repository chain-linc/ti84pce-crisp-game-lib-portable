...
void addScore(float value, float x, float y) {
  score += value;
  int v = (int)value;
  /*if (v > 9999999) { v cant get that high
    return;
  }*/
...
static void initInGame() {
  ...
  resetDrawState();
  // if (isBgmEnabled) {
  // isPlayingBgm = true; ti84pce does not have sound
  //}
  ticks = -1;
...
static void initGameOver() {
  ...
  if (isReplaying) {
    ...
    // isPlayingBgm = false; ti84pce does not have sound
    prevScore = (int)score;
  }
  gameOverTicks = 0;
  saveCurrentColorAndCharacterOptions();
...
static void resetGame(int gameIndex) {
  ...
  initScore();
  initParticle();
  // initSound(title, description, options.soundSeed); ti84pce does not have sound
  initReplay();
  parseDescription();
  setRandomSeedWithTime(&gameSeedRandom);
  md_clearScreen(options.isDarkColor ? 15 : 16);
  resetDrawState();
...
void updateFrame() {
  ...
  // updateSound(); ti84pce does not have sound
  drawScore();
  ticks++;
