...
EXTERNC void md_drawRect(float x, float y, float w, float h, unsigned char color);
EXTERNC void md_drawCharacter(
    unsigned char grid[CHARACTER_HEIGHT][CHARACTER_WIDTH], float x, float y,
    int hash);
EXTERNC void md_clearView(unsigned char color);
EXTERNC void md_clearScreen(unsigned char color);
